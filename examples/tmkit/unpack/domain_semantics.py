...
# def pick_pos(scene,name):
#     g = scene[name].collision
#     c_geo = g[0].options
#     s = g[0].shape
#     p = g[0].translation
#     if aa.shape_is_box(s):
#         return [p[0], p[1], s.dimension[2]]

def op_pick_up(scene, config, op):
    # TODO find a more precise pick pose
    (a, obj, src, i, j) = op
    ...
    x = trans[0]
    y = trans[1]
    z = 2*place_height(scene,obj) + place_height(scene,src) + EPSILON
    
    d_tf_o = aa.tf2( 1, [x,y,z])
    g_tf_d = tm.op_tf_abs(nop,src)
    g_tf_o = aa.mul(g_tf_d, d_tf_o )    
    mp = motion_plan(nop, FRAME, g_tf_o)
