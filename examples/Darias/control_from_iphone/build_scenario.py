    ...
    def reset(self):
        with HideOutput():
            with LockRenderer():
                ...
                # initial_jts = (0.7967434981466166,1.2144775140312278,-2.2945863219345437,-1.8219071566526694,-0.7339052348207756,-1.3696331213444768,-0.2325780018473118)
                config_left = BodyConf(self.arm_left, initial_jts)
                config_left.assign()
...
class ScenarioForiPhoneController(object):
    def __init__(self):
        with HideOutput():
            with LockRenderer():
                self.arm_left = load_pybullet("../darias_description/urdf/darias_L_primitive_collision.urdf",
                ...
                self.bd_body = {
                    ...
                    'c1': load_pybullet("../scenario_description/boxCm.urdf", fixed_base=False),
                    'c2': load_pybullet("../scenario_description/boxC.urdf", fixed_base=False),
                    'iphone': create_box(0.07,0.15,0.01)
                }
                self.bd_body.update(dict((self.bd_body[k], k) for k in self.bd_body))
                ...
                self.movable_bodies = [self.bd_body['c1'], self.bd_body['c2'], self.bd_body['iphone']]
                self.body_gripped = self.bd_body['c1']
                self.body_on_table = self.bd_body['c2']
                ...
                self.tcp_pose = pu.get_link_pose(self.robot, self.end_effector_link)
                self.robot_pose = pu.get_pose(self.robot)
                pu.set_pose(self.bd_body['iphone'], self.tcp_pose)
                attach_pose = ((0,0,0),(0,0,0,1))
                self.attachment = pu.Attachment(self.robot, self.end_effector_link, attach_pose, self.bd_body['iphone'])
                set_camera(150, -35, 1.6, Point(-0.1, 0.1, -0.1))
