    ...
    def __init__(self):
        with HideOutput():
            with LockRenderer():
                ...
                set_color(self.bd_body['region_drawer'], color=color)
                set_color(self.bd_body['region_table'], color=color)
                self.fb_bodies=[]
                for body_name in self.bd_body.keys():
                    if 'box' in body_name:
                        self.fb_bodies.append(self.bd_body[body_name])
                ...
                set_pose(self.bd_body['region_drawer'], Pose(Point(x=-0.02, y=0.90, z=0.001)))
                set_pose(self.bd_body['camera1'],
                         Pose(Point(x=0.20, y=1.3, z=0.9), Euler(0, 145 * np.pi / 180, -100 * np.pi / 180)))
...
class PlanningScenario_4obs_1box(object):
    def __init__(self):
        with HideOutput():
            with LockRenderer():
                self.arm_left = load_pybullet("../darias_description/urdf/darias_L_primitive_collision.urdf",
...
class PlanningScenario_4obs_2box(object):
    def __init__(self):
        with HideOutput():
            with LockRenderer():
                self.arm_left = load_pybullet("../darias_description/urdf/darias_L_primitive_collision.urdf",
                ...
                self.bd_body = {
                    ...
                    'camera1': load_pybullet("../scenario_description/realsense.urdf", fixed_base=True),
                    'box1': load_pybullet("../scenario_description/boxA.urdf", fixed_base=False),
                    'box2': load_pybullet("../scenario_description/boxA.urdf", fixed_base=False),
    ...
    def reset(self):
        with HideOutput():
            with LockRenderer():
                ...
                set_pose(self.bd_body['box1'],Pose(Point(x=-0.02, y=0.9, z=0.1), Euler(yaw=1 * np.pi / 2)))
                set_pose(self.bd_body['box2'],Pose(Point(x=0.31, y=0.8, z=0.1), Euler(yaw=1 * np.pi / 2)))
                set_pose(self.bd_body['box_S1'], Pose(Point(x=-0.02, y=0.72, z=0.1)))
                set_pose(self.bd_body['box_S2'], Pose(Point(x=-0.02, y=1.08, z=0.1)))
                set_pose(self.bd_body['box_S3'], Pose(Point(x=0.14, y=0.9, z=0.1)))
                set_pose(self.bd_body['box_S4'], Pose(Point(x=0.49, y=0.95, z=0.1)))
