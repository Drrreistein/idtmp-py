...
def get_view_aabb2(body, camera_pose, **kwargs):
    with PoseSaver(body):
        ...
        az = np.cross(ax, ay)
        view_frame = copy.copy(camera_tform)
        view_frame[:3, 0] = ax
        view_frame[:3, 1] = ay
        view_frame[:3, 2] = az
...
def get_raytest_scatter2(target_body, robot):
    ...
    bottom_center = copy.copy(obj_center)
...
def get_custom_collision_fn(robot, joints, obstacles, attachments, self_collisions, disabled_collisions,
                     custom_limits={}, **kwargs):
    check_link_pairs = get_self_link_pairs(robot, joints, disabled_collisions) \
        if self_collisions else []
    moving_links = frozenset(get_moving_links(robot, joints))
        ...
        for link1, link2 in check_link_pairs:
            ...
            if pairwise_link_collision(robot, link1, robot, link2, **kwargs):  # , **kwargs):
                print('Self collision happens at {}({}), between {}({}) and {}({}).'.format(robot, get_body_name(robot),
                                                                                            ...
                                                                                            get_link_name(robot,
                                                                                                          link2)))
                return True, {'self_collision':((robot, link1),(robot))}
        """2) Moving bodies collide with obstacles."""
        for body1, body2 in check_body_pairs:
            if pairwise_collision(body1, body2, **kwargs):
                print('body collision happens between {} and {}.'.format(body1, body2))
                return True, {'body_collision':(body1, body2)}
        return False, None
...
def plan_joint_path(robot, joints, start_conf, end_conf, obstacles=[], attachments=[],
                      self_collisions=True, disabled_collisions=set(),
                      weights=None, resolutions=None, max_distance=MAX_DISTANCE,
                      custom_limits={}, **kwargs):
...
def inverse_kinematics_random(robot, link, target_pose, obstacles, attachments, self_collisions, disabled_collisions, max_distance, max_iterations=200, custom_limits={}, movable_joints=None, **kwargs):
    ...
    collision_fn = get_collision_fn(robot, movable_joints, obstacles, attachments, self_collisions, disabled_collisions,
                                    custom_limits=custom_limits, max_distance=max_distance)

    sample_time = 0
    while sample_time<20:
        sample_conf = sample_fn()
        set_joint_positions(robot, movable_joints, sample_conf)
        # print(f"sampled conf: {sample_conf}")
...
def forward_kinematics(robot, joints, values, link):
    init_joints = get_joint_positions(robot, joints)
    set_joint_positions(robot, joints, values)
    pose = get_link_pose(robot, link)
    set_joint_positions(robot, joints, init_joints)
    return pose
...
def plan_cartesian_motion(robot, first_joint, target_link, waypoint_poses,
    ...
    for target_pose in waypoint_poses:
        for iteration in range(max_iterations):
            ...
            print(f"exceeding maximal iteration ")
            remove_body(sub_robot)
            return None
    remove_body(sub_robot)
    return solutions
