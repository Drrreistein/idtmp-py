...
def get_sucker_grasps0(body, direction, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
                       grasp_length=GRASP_LENGTH, top_offset=SIDE_HEIGHT_OFFSET):
    # TODO: compute bounding box width wrt tool frame
    # get the geometric center in body_pose frame, NOT world frame
    ...
    if direction == 0:
        ...
        for j in range(1 + under):
            ...
            rotate_z = Pose(euler=[math.pi / 2 + 0 * math.pi, 0, 0])
            grasps += [multiply(tool_pose, translate_z, rotate_z, swap_xz,
                                translate_center, body_pose)]
        ...
        for j in range(1 + under):
            ...
            rotate_z = Pose(euler=[math.pi / 2 + 1 * math.pi, 0, 0])
            grasps += [multiply(tool_pose, translate_z, rotate_z, swap_xz,
                                translate_center, body_pose)]
        ...
        for j in range(1 + under):
            ...
            rotate_z = Pose(euler=[0 * math.pi, 0, 0])
            grasps += [multiply(tool_pose, translate_z, rotate_z, swap_xz,
                                translate_center, body_pose)]
        ...
        for j in range(1 + under):
            ...
            rotate_z = Pose(euler=[1 * math.pi, 0, 0])
            grasps += [multiply(tool_pose, translate_z, rotate_z, swap_xz,
                                translate_center, body_pose)]
...
def get_sucker_grasps(body, direction, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
                      grasp_length=GRASP_LENGTH, top_offset=SIDE_HEIGHT_OFFSET):
    # TODO: compute bounding box width wrt tool frame
    # get the geometric center in body_pose frame, NOT world frame
    assert direction in {0, 1, 2, 3, 4}
    ...
    if direction == 0:
        ...
        """Back-side grasping"""
        swap_z = Pose(euler=[0, math.pi / 2, 0])
        translate_z = Pose(point=[x_offset, 0, w / 2 - grasp_length])
        for j in range(1 + under):
            rotate_z = Pose(euler=[0, 0, j * math.pi / 2])
            grasps += [multiply(tool_pose, translate_z, rotate_z, swap_z,
                                translate_center, body_pose)]
        ...
        """Left-side grasping"""
        swap_z = Pose(euler=[-math.pi / 2, 0, 0])
        translate_z = Pose(point=[x_offset, 0, l / 2 - grasp_length])
        for j in range(1 + under):
            rotate_z = Pose(euler=[0, 0, j * math.pi / 2])
            grasps += [multiply(tool_pose, translate_z, rotate_z, swap_z,
...
def get_sucker_grasps2(body, direction, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
                       grasp_length=GRASP_LENGTH, top_offset=SIDE_HEIGHT_OFFSET):
    # TODO: compute bounding box width wrt tool frame
    # get the geometric center in body_pose frame, NOT world frame
    assert direction in {0, 1, 2, 3, 4}
...
def get_sucker_grasp_top(body, direction, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
                         grasp_length=GRASP_LENGTH, top_offset=SIDE_HEIGHT_OFFSET):
    # TODO: compute bounding box width wrt tool frame
    # get the geometric center in body_pose frame, NOT world frame
    assert direction == 0
...
def get_top_cylinder_grasps(body, tool_pose=TOOL_POSE, body_pose=unit_pose(),
    ...
    for i in range(10):
        ...
        grasp = multiply(tool_pose, translate_z, rotate_z,
                         reflect_z, translate_center, body_pose)
        grasps.append(grasp)
    return grasps
...
def ray_from_pixel(camera_matrix, pixel_pos):
    """
    Return [x/z,y/z,1] of the world point wrt the pixel position.
    """
    return np.linalg.inv(camera_matrix).dot(np.append(pixel_pos, 1))
...
def get_pr2_view_section(z, camera_matrix=None):
    ...
    return [z * ray_from_pixel(camera_matrix, pos)
            for pos in pixels]
...
def is_visible_aabb(aabb, **kwargs):
    ...
    lower_z = body_lower[2]
    if lower_z < 0:
        return False
    view_lower, view_upper = get_pr2_view_section(lower_z, **kwargs)
    # TODO: bounding box methods?
    """The (x,y) of the body should be contained by the view section."""
    return not (np.any(body_lower[:2] < view_lower[:2]) or
                np.any(body_upper[:2] > view_upper[:2]))
...
def cone_mesh_from_support(bottom_vertexes):
    assert (len(bottom_vertexes) == 4)
    """Add original point as a vertex."""
    vertices = cone_vertices_from_base(bottom_vertexes)
    triangle_faces = [(1, 4, 3), (1, 3, 2)]
    for i in range(len(bottom_vertexes)):
        index1 = 1 + i
        index2 = 1 + (i + 1) % len(bottom_vertexes)
        triangle_faces.append((0, index1, index2))
...
def inverse_visibility(pr2, target_point, head_name=HEAD_LINK_NAME,
    ...
    # Z-axis. Supposed to point from the camera origin to the target object.
    ...
    """1) Initialize head conf by zeros."""
    head_conf = np.zeros(len(head_joints))
    with LockRenderer():
        with ConfSaver(pr2):
            for _ in range(max_iterations):
                set_joint_positions(pr2, head_joints, head_conf)
                head_in_world = get_link_pose(pr2, head_link)
                """2) Calculate target point wrt current head frame."""
                point_head = tform_point(invert(head_in_world), target_point)
                error_angle = angle_between(camera_axis, point_head)
                if abs(error_angle) <= tolerance:
                    break
                normal_head = np.cross(camera_axis, point_head)  # rotation axis in head frame
                normal_world = tform_point((unit_point(), quat_from_pose(head_in_world)), normal_head)  # in world frame
                correction_quat = quat_from_axis_angle(normal_world, step_size * error_angle)
                correction_euler = euler_from_quat(correction_quat)
                _, j_angular = compute_jacobian(pr2, head_link)
                """3) Evaluate how the movement caused by each head_joint matches the desired correction.
                      It is measured by vector dot: positive -> joint should move forward.
                                                    negative -> joint should reverse."""
                correction_conf = np.array([np.dot(j_angular[jt], correction_euler)
                                            for jt in movable_from_joints(pr2, head_joints)])
...
def get_view_aabb(body, view_pose, **kwargs):
    ...
    """
    with PoseSaver(body):
        body_post = get_pose(body)
        body_view = multiply(invert(view_pose), body_post)
...
def get_detection_cone(pr2, body, camera_link=HEAD_LINK_NAME, depth=MAX_VISUAL_DISTANCE, **kwargs):
    ...
    body_aabb = get_view_aabb(body, get_link_pose(pr2, head_link))  # wrt Camera Frame
    lower_z = body_aabb[0][2]
    if depth < lower_z:
        return None, lower_z
    if not is_visible_aabb(body_aabb, **kwargs):
        return None, lower_z
    support_vertexes = support_from_aabb(body_aabb)
    return cone_mesh_from_support(support_vertexes), lower_z


def get_detections(pr2, p_false_neg=0, camera_link=HEAD_LINK_NAME,
                   exclude_links=set(), color=apply_alpha(YELLOW, 0.5), **kwargs):
    """
    ...
    for body in get_bodies():
        if (body == pr2) or (np.random.random() < p_false_neg):
            continue
        """Cond 1: The body is within the view range of the camera, 
                   so that a focusing cone casting from from the camera to the body can be generated."""
        cone_mesh, z = get_detection_cone(pr2, body, camera_link=camera_link, **kwargs)
        if cone_mesh is None:
            continue
        # Generate a focusing cone body.
        cone = create_mesh(cone_mesh, color=color)
        set_pose(cone, camera_pose)
        # add_body_name(cone, name='Get detections.', lifetime=2000, color=(1, 1, 0))
        blocked_by_obtacles = any(pairwise_collision(cone, obstacles)
                                  for obstacles in set(get_bodies()) - {pr2, body, cone})
        blocked_by_links = any(link_pairs_collision(pr2, [link], cone)
                               for link in set(get_all_links(pr2)) - exclude_links)
        if not blocked_by_obtacles and not blocked_by_links:
            """Cond 2: The detection cone has no collision with other parts"""
            detections.append(Detection(body, z))
            wait_for_duration(0.5)  # Show the focusing cone.
        remove_body(cone)
    return detections


def get_visual_detections(pr2, **kwargs):
    return [body for body, _
            in get_detections(pr2, depth=MAX_VISUAL_DISTANCE, **kwargs)]
...
def visible_base_generator(robot, target_point, base_range):
    ...
    while True:
        ...
        base_xy = target_point[:2] - look_distance * base_from_table  # position in x-y plane
        base_theta = np.math.atan2(base_from_table[1], base_from_table[0])  # orientation
